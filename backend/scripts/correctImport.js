...
async function correctImport() {
  console.log('🔄 Reimportación con codificación correcta (latin1)...\n');
  ...
  try {
    ...
    // 2. Leer CSV con codificación latin1
    const csvPath = path.join(__dirname, '../../DocumentosExcel/LISTA DE PRECIOS GEOFAL 2025 NUEVO.csv');
    console.log('📁 Leyendo CSV con codificación latin1:', csvPath);
    ...
    const examples = await pool.query(`
      ...
      FROM ensayos 
      WHERE codigo IN ('SU04', 'SU19', 'AG08A', 'AG09', 'SU22', 'EE01', 'EE02', 'ALB01')
      ORDER BY codigo
    `);
