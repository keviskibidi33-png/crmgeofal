...
import { FiUser, FiFileText, FiCheck, FiArrowRight, FiPlus, FiTrash2, FiEye, FiSave } from 'react-icons/fi';
import { useAuth } from '../contexts/AuthContext';
import { createQuote } from '../services/quotes';
import { getOrCreateCompany, listCompanies } from '../services/companies';
import { createProject } from '../services/projects';

const CotizacionInteligenteMejorada = () => {
  const { user } = useAuth();
  ...
  const [saveOption, setSaveOption] = useState('intelligent'); // intelligent, personal, shared
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      const project = await createProject({
        name: formData.project.name || `Proyecto - ${formData.client.company_name}`,
        description: formData.project.description,
        location: formData.client.project_location,
        company_id: company.id
      });

      // Crear cotización como borrador
      ...
      const quote = await createQuote({
        ...
        igv: totals.igv,
        total: totals.total,
        status: 'borrador', // Siempre como borrador
      ...
      setSuccess(`¡Borrador guardado exitosamente! ID: ${quote.id}`);
  ...
  const handleSaveAndApprove = async () => {
    try {
      ...
      const quote = await createQuote({
        ...
        igv: totals.igv,
        total: totals.total,
        status: 'aprobada', // Aprobada directamente
        reference: formData.quote.reference,
        meta: {
          customer: formData.client,
          quote: formData.quote,
      ...
      // Notificar que la cotización fue aprobada
      // (Esto activará las notificaciones automáticas)

      setSuccess(`¡Cotización creada y aprobada exitosamente! ID: ${quote.id}`);
...
export default CotizacionInteligenteMejorada;
